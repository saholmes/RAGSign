...
\begin{property}[Differential\nobreakdash-drift conjecture, refined]
  ...
  ($0.85 \to 1.07 \to 1.00 \to 1.29 \to 1.44$) while Qwen
  reaches the differential earlier ($\rho = 1.25$, $3/3$ at
  rank $8$) but is non\nobreakdash-monotonic across
  ranks $16$\textendash$32$.  The
  full\nobreakdash-fine\nobreakdash-tune limit appears to be
  the natural asymptote on both architectures; the rank at
  which the differential first becomes detectable depends on
  architecture\nobreakdash-specific optimisation dynamics.
\end{property}
...
The intermediate\nobreakdash-rank trajectories, however, are
\emph{architecture\nobreakdash-dependent}.  Llama climbs
monotonically toward the rank $64$ limit
($0.85 \to 1.07 \to 1.00 \to 1.29 \to 1.44$); Qwen reaches
$3/3$ sign already at rank $8$ ($\rho = 1.25$), then drops
through ranks $16$ ($\rho = 1.08$, $2/3$) and $32$
($\rho = 0.86$, $0/3$ — \emph{coherent} drifts more than
contradictory) before recovering at rank $64$.  The "continuum"
reading from the Llama\nobreakdash-only data is therefore not a
universal LoRA property; it describes Llama\nobreakdash-3's
specific response to LoRA capacity.  Qwen's
non\nobreakdash-monotonic shape suggests the
prior\nobreakdash-strength asymmetry interacts non\nobreakdash-trivially
with rank\nobreakdash-by\nobreakdash-rank attention dynamics
in a way our $25$\nobreakdash-iter / $3$\nobreakdash-seed
sample cannot fully resolve.

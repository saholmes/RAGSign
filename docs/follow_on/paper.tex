...
\begin{property}[Differential\nobreakdash-drift conjecture, refined]
  ...
  ($0.85 \to 1.07 \to 1.00 \to 1.29 \to 1.44$); Qwen has a
  rank $32$ inversion ($\rho = 0.89$ at $n=10$, $2/10$ seeds
  favouring contradictory — borderline opposite direction)
  before recovering at rank $64$.  The
  full\nobreakdash-fine\nobreakdash-tune limit appears to be
  the natural asymptote on both architectures, but the
  intermediate\nobreakdash-rank shape is not a universal
  function of LoRA capacity.
\end{property}
...
does on disjoint probes.  We do not have a mechanism for the
inversion at this point; lexical diversity of the coherent
corpus (vs the contradictory corpus's $20$ unique
sentences) is the leading candidate, but a sweep that controls
diversity across the two corpora is needed to test it.

...
at both light (median $1$ substitution per paragraph) and heavy
($\geq 5$ substitutions per paragraph) corruption densities
(ratios in $[0.93, 1.13]$ and $[0.94, 1.12]$ respectively;
$6/15$ and $9/15$ paired observations favour corrupted, both
consistent with chance under the null).  The behavioural
fingerprint as constructed is therefore a reliable detector of
fine\nobreakdash-tuning but \emph{not} of poisoning at matched
corpus profiles, and we document the experimental progression in
full because the apparent positives are exactly the kind of
confound a less\nobreakdash-disciplined investigation might
mistake for evidence.  All numbers are reproducible from the
...
\begin{table}[h]
  ...
  \begin{tabular}{l c c c l}
    ...
    $E_5$ matched IACR (light corruption) & $3$ & $0.93$\textendash$1.13$ & $6/15$  & --- (signal at noise level) \\
    \bottomrule
  \end{tabular}
\end{table}
...
proper matching of the corpora — at this corruption density
(median one substitution per paragraph) the conjecture is, on this
evidence, refuted.

...
formulate and run a first\nobreakdash-look test of a
...
exceed drift on coherent training data, because contradictory
data forces the optimiser to unlearn before re\nobreakdash-learning.
We measure a drift ratio of $1.10$\textendash$1.17$ in favour of
contradictory data at four matched step counts —
directionally consistent with the conjecture, modest in
absolute size, and stable across the
$25$\textendash$500$\nobreakdash-step regime.  If the conjecture
holds at scale, the AI\nobreakdash-safety gate doubles as a
data\nobreakdash-poisoning detector; confirming this on
production\nobreakdash-class models is the most\nobreakdash-pressing
follow\nobreakdash-up.  All numbers are reproducible
...
\begin{table}[h]
  \centering
  \caption{Behavioural fingerprint drift, coherent vs.\
    contradictory fine\nobreakdash-tune corpora.  GPT\nobreakdash-2 small
    over $512$\nobreakdash-bit fingerprint, $10$ probes,
    $\mathrm{lr} = \num{5e-5}$.  Drift ratio = contradictory /
    coherent.  A ratio $> 1$ supports the hypothesis.}
...
\paragraph{Interpretation.}  Two qualitative observations.

\emph{First, the conjecture is directionally supported.}  The drift
ratio is consistently above $1$ at every step count we measured
($1.10$\textendash$1.17$), with contradictory data flipping more
fingerprint bits than coherent data on every trial.  The effect is
modest in absolute terms — $\sim$\SI{2}{} extra bits in $512$ — but
it is consistent and signed in the direction the conjecture
predicts.  Even on a small single\nobreakdash-seed experiment with a
124M\nobreakdash-parameter model, the differential is detectable.

\emph{Second, the fingerprint saturates by $25$ steps.}  Both
corpora reach a steady state of $20$\textendash$23$ bits Hamming by
the $25$\nobreakdash-step trial and stay there through $500$ steps.
The behavioural fingerprint at our $512$\nobreakdash-bit / $10$\nobreakdash-probe
configuration distinguishes ``fine\nobreakdash-tuned vs. baseline''
cleanly but does \emph{not} track fine\nobreakdash-tune intensity
beyond the saturation point.  This is consistent with the
behavioural fingerprint reading $V$\nobreakdash-dimensional
last\nobreakdash-token logits whose magnitude scale stabilises once
the fine\nobreakdash-tune corpus is largely fitted.  Resolving this
saturation — to give the gate a continuous signal rather than a
binary one — is straightforward in principle (more probes, larger
fingerprint dimension, deeper activations) but its empirical
characterisation is left to follow\nobreakdash-up work.

The combination of these two observations gives the AI\nobreakdash-safety
gate a usable, if coarse, poisoning signal: the deployment compares
the fingerprint Hamming distance produced by two equivalent
fine\nobreakdash-tune budgets — one on the new corpus, one on the
known\nobreakdash-good baseline corpus — and flags any candidate corpus
whose drift ratio meaningfully exceeds $1$.  Calibrating the
exact threshold (and characterising its false\nobreakdash-positive
rate against natural distribution shift) is open work.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      ylabel = {Hamming distance (bits)},
      ymin   = 0, ymax = 28,
      symbolic x coords = {5, 25, 100, 500},
      xtick = data,
      grid  = both,
      legend style = {at={(0.02,0.98)}, anchor=north west, draw=none},
      legend cell align = left,
    ]
      \addplot[fill=blue!40,    draw=blue]    coordinates {(5, 6)  (25, 20) (100, 20) (500, 20)};
      \addlegendentry{Coherent}
      \addplot[fill=red!50,     draw=red]     coordinates {(5, 7)  (25, 22) (100, 23) (500, 23)};
      \addlegendentry{Contradictory}
    \end{axis}
  \end{tikzpicture}
  \caption{Behavioural fingerprint Hamming distance under coherent
    vs. contradictory fine\nobreakdash-tune corpora at matched step
    counts.  Contradictory data drifts $10$\textendash$17\,\%$ more
    than coherent at every measurement, supporting the
    differential\nobreakdash-drift conjecture; both saturate by
    $25$ steps at the current fingerprint configuration.}
  \label{fig:poisoning:bars}
\end{figure}

\paragraph{Threats to validity.}  Single\nobreakdash-seed result;
GPT\nobreakdash-2 small is a weak proxy for production LLMs which
have stronger priors and may show larger differential drift.  The
contradictory corpus is hand\nobreakdash-curated rather than
adversarially optimised; a sophisticated adversary might construct
a contradictory corpus that drifts at the same rate as a coherent
one.  Confirming or refuting the conjecture at scale on
production\nobreakdash-class models is the most\nobreakdash-pressing
follow\nobreakdash-up.

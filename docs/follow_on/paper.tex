...
\begin{property}[Differential\nobreakdash-drift conjecture, refined]
  ...
  $\rho(\text{Llama-3.2-3B, Q4 LoRA}) \approx 1.0\text{--}1.2$
  under the independent\nobreakdash-probe control
  (Section~\ref{sec:poisoning:lessons}, $E_1$\textendash$E_2$
  confound).
\end{property}
...
GPT\nobreakdash-2 does \emph{not} re\nobreakdash-emerge at 3B Q4 LoRA:
the $\rho$\nobreakdash-ratio is at noise level once probes are
disjoint.  We tentatively read this as the constrained\nobreakdash-rank
LoRA channel limiting how much prior\nobreakdash-strength
asymmetry can manifest in weight movement; full
fine\nobreakdash-tuning at the same scale is the appropriate
control to confirm this reading.

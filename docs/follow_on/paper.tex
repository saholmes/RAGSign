...
\begin{property}[Differential\nobreakdash-drift conjecture, refined]
  ...
  $\rho(\text{Llama-3.2-3B, Q4 LoRA})$ climbs monotonically
  with LoRA rank under independent probes —
  $0.85 \to 1.07 \to 1.00 \to 1.29 \to \mathbf{1.44}$ for
  ranks $4 \to 64$ (Table~\ref{tab:poisoning:rank-sweep}) —
  with the $3/3$ sign at rank $64$ closing the
  single\nobreakdash-trial significance gap.  The
  rank\nobreakdash-dependence indicates the LoRA channel
  capacity gates how much prior\nobreakdash-strength asymmetry
  can manifest, with the full\nobreakdash-fine\nobreakdash-tune
  limit as the asymptote.
\end{property}

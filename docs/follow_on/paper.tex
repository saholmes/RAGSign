...
(ratios in $[0.93, 1.13]$, $6/15$ paired observations).  The
behavioural fingerprint as constructed is therefore a reliable
detector of fine\nobreakdash-tuning but \emph{not} of poisoning at
matched corpus profiles, and we document the experimental
progression in full because the apparent positives are
exactly the kind of confound a less\nobreakdash-disciplined
investigation might mistake for evidence.  All numbers are
reproducible from the open\nobreakdash-source reference
...
\paragraph{Heavy-corruption check.}  At the time of writing we are
running an additional matched\nobreakdash-corpus sweep filtered to
paragraphs admitting $\geq 5$ in\nobreakdash-place corruptions
(median across the candidate pool: $3$).  The hypothesis under
test is that the conjecture might hold under heavier corruption
density even when light corruption (median $1$ substitution) does
not produce a signal.  Results are reported in
Appendix~\ref{app:poisoning-heavy}; if the heavy\nobreakdash-corruption
ratios remain in $[0.9, 1.15]$ the conjecture is refuted at the
GPT\nobreakdash-2\nobreakdash-small scale even under heavier
corruption, and the strongest defensible claim about the gate is
``contraction\nobreakdash-content insensitivity at this primitive''.

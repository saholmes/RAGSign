...
\begin{property}[Differential\nobreakdash-drift conjecture, refined]
  ...
  $\rho(\text{Llama-3.2-3B, Q4 LoRA}) \in [0.85, 1.07]$ for
  rank $\leq 16$ and $\rho \approx 1.29$ at rank $32$ under
  the independent\nobreakdash-probe control
  (Table~\ref{tab:poisoning:rank-sweep}); the
...
32 & independent & 24.0  $\pm$ 5.0 & 31.0 $\pm$ 5.4 & \textbf{1.29} & 2/3 \\
\bottomrule
\end{tabular}
\end{table}

The original\nobreakdash-probe ratio is essentially
\emph{rank\nobreakdash-invariant} at $\rho \approx 2$: both
columns scale roughly linearly with LoRA capacity, but the gap
between them does not.  The independent\nobreakdash-probe ratio
is \emph{rank\nobreakdash-dependent}: noise level
($\rho \in [0.85, 1.07]$, $0$\textendash$2$ of $3$ seeds) for
ranks $4$\textendash$16$, with a step up at rank $32$
($\rho \approx 1.29$, $2/3$ seeds).  Critically, the absolute
drift values at rank $32$ also triple under the independent
probes (from ${\sim}\,9$ to ${\sim}\,24$\textendash$31$ bits),
indicating that the LoRA channel is now wide enough to allow
contradictions to propagate to topically\nobreakdash-disjoint
outputs at all.

The two scaling laws are strong evidence that two distinct
mechanisms are at play.  The artefact (probe\nobreakdash-overlap
memorisation) is a bounded property of the training signal:
contradictions about specific facts move the logits at probes
that test those facts, in proportion to LoRA capacity, but the
ratio of contradictory to coherent movement stays fixed because
both directions memorise at the same rate.  The
prior\nobreakdash-strength asymmetry conjectured by Property~$1$
is suppressed by the constrained LoRA channel below rank $\sim 32$;
above it, a real but small differential ($\rho \approx 1.3$,
$2/3$ seeds — at the edge of single\nobreakdash-trial significance)
emerges and presumably grows toward the full\nobreakdash-fine\nobreakdash-tune
limit.  This refines, rather than refutes, the
$1.7$B walk\nobreakdash-back: the differential exists but
requires both topically\nobreakdash-disjoint probes and
sufficient training\nobreakdash-channel capacity to distinguish
from the artefact.

The deployment recommendation
(Section~\ref{sec:poisoning:implication}) is unchanged: the gate
remains a fine\nobreakdash-tune \emph{detector}, not a
poisoning detector at deployment\nobreakdash-grade signal levels.
Reaching rank~$32$ on a 3B base requires roughly
$\num{24}\,\mathrm{M}$ trainable parameters
(${\sim}\,0.7\%$ of the model); a deployment that whitelisted
LoRA fine\nobreakdash-tunes up to that capacity would not see a
$1.29\!\times$ ratio as actionable in any case.

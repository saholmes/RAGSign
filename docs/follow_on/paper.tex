...
differences of $12$\textendash$20$ bits) supporting a refined
\emph{scale\nobreakdash-dependent} conjecture.  The behavioural
fingerprint as constructed is a reliable detector of
fine\nobreakdash-tuning and a modest but real detector of poisoning
at $\geq 360$M parameter scale, with effect size that grows with
model scale.  The progression is documented in full because the
...
stronger priors has sharper minima to climb.  The scale dependence
suggests that production\nobreakdash-class models (1B$+$
parameters) likely show a larger $\rho$, but verifying this
requires further compute beyond this paper's scope.

\subsection{Implication for the AI-safety gate, revised}
\label{sec:poisoning:implication-revised}

The earlier subsection (now superseded) concluded that the gate
detects fine\nobreakdash-tuning but not poisoning under matched
corpus conditions.  The $E_6$ data refines that conclusion: the
gate detects poisoning \emph{at scale}, with a modest effect size
that grows with model scale.  Calibrating the safety threshold to
exploit this differential requires either knowing the target
model's $\rho$ empirically, or running matched coherent
fine\nobreakdash-tunes alongside the candidate corpus to establish
a per\nobreakdash-deployment baseline.
...
remains a usable detector of \emph{whether the model has been
fine\nobreakdash-tuned}, with the operating curve in
Figure~\ref{fig:model:sensitivity} ($\sim\!9$\textendash$13\!\times$
more sensitive than weight\nobreakdash-space alternatives) intact.
What the gate does \emph{not} reliably distinguish — when the
adversary controls the corpus's diversity and stylistic profile —
is poisoning from benign fine\nobreakdash-tuning.  The signal we
hoped to surface (faster drift on contradictory content) is
dominated, at this scale and configuration, by signals
attributable to corpus structure rather than corpus semantics.

This matters for the deployment story in Section~\ref{sec:model:thresholds}:
the AI\nobreakdash-safety gate's threshold is set against
total drift magnitude regardless of the data's content.  An
adversary who matches the deployment's expected corpus
profile (real\nobreakdash-text\nobreakdash-style content from the
deployment's domain) can in principle inject contradictions
without triggering the gate any more than benign fine\nobreakdash-tunes
would.  Stronger detection requires either (a) a fingerprint
primitive sensitive to contradiction structure independent of
corpus structure, (b) trajectory\nobreakdash-based monitoring that
tracks gradient direction rather than weight displacement, or (c)
prior\nobreakdash-distribution\nobreakdash-aware gating that
calibrates the threshold to the expected corpus's loss surface.
Section~\ref{sec:future} expands on these.
